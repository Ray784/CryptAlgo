...
class Main{
	private static class BlowFish{
	    ...
	    String P[] = { "243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822", "299f31d0", "082efa98", "ec4e6c89", "452821e6", "38d01377", "be5466cf", "34e90c6c", "c0ac29b7", "c97c50dd", "3f84d5b5","b5470917", "9216d5d9", "8979fb1b" };
		...
		void updateKeys(){
			...
			for(int i = 0; i < 18; i += 2){
				...
				P[i] = input.substring(0, 8);
				P[i + 1] = input.substring(8, 16);
			}
		}

		void updateSBox(int num, String input){
			for(int i = 0; i < 256; i += 2){
				input = encipher(input);
				sbox[num][i] = input.substring(0, 8);
				sbox[num][i+1] = input.substring(8, 16);
			}
		}
		...
		String round(String text, String key){
		...
		void genKeys_Sbox(String key){
			initKeys(key);
			updateKeys();
			updateSBox(0, P[16]+P[17]);
			for(int i = 1; i < 4; i++)
				updateSBox(i, sbox[i-1][254]+sbox[i-1][255]);
		}

		String encrypt(String plainText, String key){
			genKeys_Sbox(key);
			plainText = encipher(plainText);
			return plainText;
		}

		String decrypt(String plainText, String key){
			//genKeys_Sbox(key);
			plainText = decipher(plainText);
			return plainText;
		}
	}
